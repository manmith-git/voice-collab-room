...
function activate(context) {
  context.subscriptions.push(
    vscode.commands.registerCommand('voiceRoom.open', () => {
      ...
      panel.webview.html = html.replace(/__SIGNALING_URL__/g, process.env.SIGNALING_URL || 'https://yourapp.onrender.com:3000');

      panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.type === 'open-mic') {
          // open external mic helper with name & ids
          const micUrlBase = process.env.MIC_HELPER_URL || 'https://yourapp.onrender.com:8000/mic-helper/index.html';
